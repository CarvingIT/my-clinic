<x-app-layout>
            ...
            <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-sm sm:rounded-lg p-4 md:p-6">
                <form method="POST" action="{{ route('groups.update', $group->id) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $group->name) }}" required placeholder="e.g. Sharma Family"
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3" placeholder="Additional notes about the group or family"
                            class="w-full border-gray-300 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('description', $group->description) }}</textarea>
                    ...
                    <div class="flex gap-2 pt-4">
                        <button type="submit" class="bg-indigo-600 text-white rounded-md px-4 py-2 text-sm font-semibold hover:bg-indigo-700 shadow transition duration-150">Update Group</button>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const initialMembers = @json($group->members);
            initialMembers.forEach(member => {
                addPatientToGroup(member);
            });
        });
    </script>
</x-app-layout>
